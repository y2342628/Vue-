<template>
	<div class="marTmp">
		...
		<!-- 每块 的标题 -->
		<el-row class='title'>
		  <el-col :span="18"><h3>{{moveTitle[moveType]}}</h3></el-col>
...
<script>
	export default{
		...
		created(){
			this.moveType=this.$route.params.moveType
			this.getlist()
		},
		methods:{
			//发送ajax获得数据
			getlist(){
				this.$http.jsonp('http://api.douban.com/v2/movie/'+this.moveType,{params:{start:this.start,count:10}})
				.then(function(res){
